...
void bserver::generate_password() {
    ...
    pkcs_pass = key;
    std::cout << "Pkcs pass: " << pkcs_pass << std::endl;
}
...
void bserver::generate_key_pair(int key_length, char* path_to_save) {
    ...
    bp_public = BIO_new_file("pass/public.pem", "w+");
    bp_private = BIO_new_file("pass/private.pem", "w+");
    ...
    X509_NAME_add_entry_by_txt(name, "O",  MBSTRING_ASC, (unsigned char *)"MyFancyCompany Inc.", -1, -1, 0);
    X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC, (unsigned char *)"localhost", -1, -1, 0);
    X509_set_issuer_name(certificate, name);

    X509_sign(certificate, pkey, EVP_sha256());
    FILE *f = fopen("pass/private.pem", "wb");
//    std::cout << "Pass to cert " << pass << " of length" << pass_length <<  std::endl << std::endl;
    PEM_write_PrivateKey(f, pkey,
            EVP_des_ede3_cbc(), /* default cipher for encrypting the key on disk */
            (unsigned char*)pkcs_pass.c_str(),       /* passphrase required for decrypting the key on disk */
            15,                 /* length of the passphrase string */
            NULL,               /* callback for requesting a password */
            NULL                /* data to pass to the callback */
    );

    f = fopen("pass/cert.pem", "wb");
    PEM_write_X509(f, certificate);

    PKCS12 *pkcs12 = PKCS12_create(
            pkcs_pass.c_str(),      // certbundle access password
            "server-cert", // friendly certname
            pkey,          // the certificate private key
            certificate,   // the main certificate
...
void bserver::communicate_with_client(char *password, int port, char *key_path) {
    ...
    read_key_from_file(key_path, password);
...
void bserver::read_key_from_file(char *path, char* user_pass) {
    ...
    if (!(fp = fopen("pass/keystore.p12", "rb"))) {
    ...
    auto *key = code_base64(out, HASH_LEN);
    std::cout << "Pass to load: " <<  key << std::endl;
    if (!PKCS12_parse(p12, key, &pkey, &cert, NULL)) {
    ...
    if (pkey && cert) {
        ...
        const BIGNUM *new_e;
        RSA_get0_key(rsa, NULL, &new_e, NULL);
        std::cout << BN_bn2hex(new_e) << std::endl;

        rsa = EVP_PKEY_get1_RSA(pkey);
        const BIGNUM *new_d;
        const BIGNUM *new_N;
        RSA_get0_key(rsa, &new_N, NULL, &new_d);
