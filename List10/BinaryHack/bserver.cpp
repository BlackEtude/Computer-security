...
void bserver::communicate_with_client(int port, char *key_path) {
    // If pass is not correct -> end
//    char *p;
//    if(!is_server_password_valid(p)) {
...
//    memset(out, 0, sizeof out);
    ...
    // Hash user's pass
//    if(PKCS5_PBKDF2_HMAC(given_pass, LENGTH, reinterpret_cast<const unsigned char *>(salt.c_str()), LENGTH, ITERS, EVP_sha256(), HASH_LEN, out) != 1)
//        std::cout << "Failure" << std::endl;
//    char *key = code_base64(out, HASH_LEN);
//    char p[45];
//    strcpy()

    if(strcmp(given_pass, "+gMHKqv1YO5Rq5+AgbJ0uxSEQwktpAOV3PocgAFJH2Q=")) {
        printf ("\n Wrong Password \n");
    }
    else {
        printf ("\n Correct Password \n");
...
int main(int argc, char*argv[]) {
    ...
    if(strcmp(argv[1], "sign") == 0) {
        ...
        server->communicate_with_client(8080, const_cast<char *>("key/prvivate2048"));
    }
